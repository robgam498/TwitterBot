...
public class TwitterBot {
    ...
    public TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
        ...
        List<List<String>> td = tp.csvDataToTrainingData(br, tweetColumn);

        for (List<String> sentence : td) {
            Iterator<String> words = sentence.iterator();
            mc.train(words);
        }
    }

    public TwitterBot(String emptyData, int tweetColumn, ListNumberGenerator num) {
    }
    ...
    public void writeStringsToFile(
        ...
        try {
            ...
            throw new RuntimeException("IO Caught");
        }
    }
    ...
    public String generateTweet(int numWords) {
        ...
        String result = "";
        if (!mc.hasNext() || numWords == 0) {
            return result;
        }
        String finished = "";
        int total = 1;
        while (total < numWords + 1) {
            if (!mc.hasNext()) {
                result = result + randomPunctuation();
                mc.reset();
            }
            finished = mc.next();
