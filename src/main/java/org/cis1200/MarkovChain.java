...
public class MarkovChain implements Iterator<String> {
    ...
    public void train(Iterator<String> sentence) {
        ...
        if (sentence.hasNext()) {
            ...
            startWords.record(result);

            while (sentence.hasNext()) {
                next = sentence.next();
                addBigram(result, next);
                result = next;
            }
            addBigram(result, END_TOKEN);
        }
    }
    ...
    public boolean hasNext() {
        return (!(currMsg == null || currMsg.equals(END_TOKEN) || currMsg.isEmpty()));
    }
    ...
    public String next() {
        ...
        String result = currMsg;
        ProbabilityDistribution<String> p = chain.get(currMsg);
        if (p == null) {
            currMsg = null;
        } else {
            currMsg = p.pick(ng);
