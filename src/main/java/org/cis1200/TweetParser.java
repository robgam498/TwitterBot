...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        if (csvLine == null || csvColumn < 0) {
            ...
            try {
                String[] lSplit = csvLine.split(",");
                return lSplit[csvColumn];
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        FileLineIterator itt = new FileLineIterator(br);
        String s;
        List<String> result = new ArrayList<>();
        while (itt.hasNext()) {
            s = itt.next();
            if (!(extractColumn(s, tweetColumn) == null)) {
                result.add(extractColumn(s, tweetColumn));
            }
        }
        return result;
    }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        List<String> original = tweetSplit(removed);
        List<String> clean;
        List<List<String>> finished = new ArrayList<>();

        for (String s : original) {
            clean = parseAndCleanSentence(s);
            if (clean.size() > 0) {
                finished.add(clean);
            }
        }
    ...
    public static List<List<String>> csvDataToTrainingData(BufferedReader br, int tweetColumn) {
        List<List<String>> finished = new ArrayList<>();
        List<String> original = csvDataToTweets(br, tweetColumn);

        for (String tweet : original) {
            finished.addAll(parseAndCleanTweet(tweet));
        }
        return finished;
    }
